...
async fn should_create_payout_recipient() {
    let mut payment_info = WiseTest::get_payment_info();
    payment_info = payment_info.map(|mut pi| {
        pi.payout_method_data = Some(api::PayoutMethodData::Bank(api::payouts::BankPayout::Bacs(
    ...
    let response = CONNECTOR
        .create_payout_recipient(enums::PayoutType::Bank, payment_info)
        .await
...
async fn should_create_bacs_payout() {
    ...
    let recipient_res = CONNECTOR
        .create_payout_recipient(enums::PayoutType::Bank, payment_info.to_owned())
        .await
